...
def load_source_config(path: Path) -> dict[str, dict]:
    """{outlet_domain: {lang, region, alignment, credibility}} from config/sources.yml.
    Missing/unparseable file → empty dict (outlets simply get no alignment tag)."""
    p = Path(path)
    if not p.exists():
        return {}
    try:
...
class TavilySearch:
    ...
    def __init__(self, depth: str = "advanced", max_results: int = 12, source_config: dict | None = None):
    ...
    def search(self, query: str, lang: str = "en", max_results: int | None = None) -> list[RawItem]:
        ...
        for r in resp.get("results", []):
            ...
            out.append(RawItem(
                ...
                outlet=outlet,
                lang=lang,
                alignment=self._source_config.get(outlet, {}).get("alignment"),
            ))
        return out
...
def get_search(settings) -> SearchClient:
    if settings.search_backend == "tavily":
        source_config = load_source_config(settings.sources_yml)
